...
class Migration:
    def forwards(self, orm):
        for slug in orm.Student.objects.values_list('slug', flat=True).distinct():
            student = orm.Student.objects.filter(slug=slug).order_by('pk')[0]

            orm.Userset.objects.filter(student__slug=slug).update(student=student)

            orm.Student.objects.exclude(pk=student.pk).filter(slug=slug).delete()

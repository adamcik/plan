...
class Migration:
    ...
    def forwards(self, orm):
        ...
        for course in orm.Course.objects.all():
            for semester in course.semesters.all():
                ...
                orm.Exam.objects.filter(course=course, semester=semester).update(course=new_course)
    ...
    def backwards(self, orm):
        ...
        for course in orm.Course.objects.filter(semester=None):
            for semester in course.semesters.all():
                exams = orm.Exam.objects.filter(course__name=course.name, semester=semester)
                lectures = orm.Lecture.objects.filter(course__name=course.name, semester=semester)
                usersets = orm.UserSet.objects.filter(course__name=course.name, semester=semester)


                if exams:
                    exams.update(course=course)
                if lectures:
                    lectures.update(course=course)
                if usersets:
